...
class ChatInputHandler():
    ...
    def construct_message(self, event_data):
        ...
        constructed_message_content = f"Timestamp: {str(format_timestamp)}, [username]: {str(event_data.user_name)}, [user id]: {str(event_data.user_id)}, [user email]: {event_data.user_email}, [Directly mentioning you]: {str(event_data.is_mention)}, [THREAD REFERENCE]: {str(event_data.channel_id)}-{str(event_data.thread_id)} [message]: {str(event_data.text)}"
