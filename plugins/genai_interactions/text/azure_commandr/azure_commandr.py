...
class AzureCommandrConfig(BaseModel):
    PLUGIN_NAME: str
    AZURE_COMMANDR_INPUT_TOKEN_PRICE: float
    AZURE_COMMANDR_OUTPUT_TOKEN_PRICE: float
    AZURE_COMMANDR_KEY: str
    AZURE_COMMANDR_ENDPOINT: str
    AZURE_COMMANDR_MODELNAME: str

class AzureCommandrPlugin(GenAIInteractionsTextPluginBase):
    def __init__(self, global_manager: GlobalManager):
        ...
        azure_commandr_config_dict = global_manager.config_manager.config_model.PLUGINS.GENAI_INTERACTIONS.TEXT["AZURE_COMMANDR"]
    ...
    def initialize(self):
        # Client settings
        self.azure_commandr_key = self.azure_commandr_config.AZURE_COMMANDR_KEY
        self.azure_commandr_endpoint = self.azure_commandr_config.AZURE_COMMANDR_ENDPOINT
    ...
    def load_client(self):
        try:
            self.commandr_client = AsyncOpenAI(
                base_url=self.azure_commandr_endpoint, api_key=self.azure_commandr_key
            )
            ...
            self.logger.error(f"Unexpected error while loading Azure CommandR client: {e}")
            raise
    ...
    async def generate_completion(self, messages, event_data: IncomingNotificationDataBase, raw_output=False):
        ...
        try:
            ...
            completion = await self.commandr_client.chat.completions.create(
                model=self.azure_commandr_modelname,
                messages=messages,
                temperature=0.1,
                top_p=0.1,
            )
