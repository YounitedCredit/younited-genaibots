        ...
        try:
            ...
            if not message_type:
                ...
                await self.user_interactions_dispatcher.send_message(event=event,
                                                                     ...
                                                                     message_type=MessageType.TEXT, is_internal=False,
                                                                     action_ref="call_subprompt")
                return None
            else:
                await self.user_interactions_dispatcher.send_message(event=event,
                                                                     message=f"Invoking subprompt: [{message_type}]...",
            ...
            if category and sub_category:
                self.logger.info(
                    f"Feedback is enabled for this subprompt: {message_type} with category: {category} and subcategory: {sub_category}")
            ...
            if subprompt is not None:
                if feedbacks:
                    await self.user_interactions_dispatcher.send_message(event=event,
                                                                         message=f"Trigger subprompt [{message_type}] with feedback [{category}-{sub_category}]",
                                                                         message_type=MessageType.COMMENT,
                                                                         is_internal=False, action_ref="call_subprompt")
                    ...
                    await self.user_interactions_dispatcher.send_message(event=event,
                                                                         message=f"Trigger subprompt [{message_type}]",
                                                                         message_type=MessageType.COMMENT,
                                                                         is_internal=False, action_ref="call_subprompt")
                    self.logger.info(f"launching completion on updated system prompt {message_type}")
