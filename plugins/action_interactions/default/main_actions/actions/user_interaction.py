...
class UserInteraction(ActionBase):
    ...
    async def execute(self, action_input: ActionInput, event: IncomingNotificationDataBase):
        ...
        if not message:
            ...
            if as_file == 'true':
                if is_custom_target:
                    await self.user_interactions_dispatcher.upload_file(event=event_copy, file_content=message,
                                                                        filename=title, title=title)
                else:
                    await self.user_interactions_dispatcher.upload_file(event=event, file_content=message,
                                                                        filename=title, title=title)
            else:
                if is_custom_target:
                    await self.user_interactions_dispatcher.send_message(event=event_copy, message=message,
                                                                         message_type=MessageType.TEXT,
                                                                         action_ref="user_interaction")
                else:
