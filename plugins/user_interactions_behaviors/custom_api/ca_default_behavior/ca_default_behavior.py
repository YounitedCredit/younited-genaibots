...
class CaDefaultBehaviorPlugin(UserInteractionsBehaviorBase):
    ...
    async def process_interaction(self, event_data, event_origin=None):
        try:
            ...
            event: IncomingNotificationDataBase = await self.user_interaction_dispatcher.request_to_notification_data(
                event_data, plugin_name=event_origin)
            if event is None:
            ...
            await self.backend_internal_data_processing_dispatcher.write_data_content(processing_container,
                                                                                      session_name, data="processing")
            ...
            if event.event_label == "thread_message":
                if break_keyword in event.text:
                    ...
                    self.logger.info(
                        f"IM behavior: Break keyword detected in thread message, stopping processing with flag {abort_name}.")
                    ...
                    abort_name = f"{str(channel_id).replace(':', '_')}-{thread_id}.txt"
                    self.logger.info(
                        f"IM behavior: Start keyword detected in thread message, resuming processing with flag {abort_name}.")
                    await self.user_interaction_dispatcher.send_message(
                        event=event,
                    ...
                    self.logger.info(
                        f"IM behavior: Clear keyword detected in thread message, clearing the queue for {channel_id} {thread_id}.")
                    await self.user_interaction_dispatcher.send_message(
                        event=event,
            ...
            if event.event_label == "thread_message" and not event.is_mention and require_mention_thread_message:
                ...
                if require_mention_new_message and not event.is_mention:
                    self.logger.info(
                        "IM behavior: Event is a new message without mention and mentions are required, not processing.")
                    return
                if not require_mention_new_message:
                    ...
                    self.logger.info(
                        "IM behavior: Event is a new message without mention and non-processed messages are not recorded, not processing.")
                    return

            # Check if there are pending messages in the queue for this event's channel/thread
            self.logger.info(
                f"IM behavior: Checking for pending messages in channel '{event.channel_id}' and thread '{event.thread_id}'")
            if await self.backend_internal_queue_processing_dispatcher.has_older_messages(
                ...
                if self.global_manager.bot_config.ACTIVATE_MESSAGE_QUEUING:
                    ...
                    self.logger.info(
                        f"IM behavior: Message from channel {event.channel_id} enqueued due to pending messages.")
                    if event.event_label == "message":
                        self.logger.warning(
                            f"IM behavior: Message from channel {event.channel_id} discarded due to pending messages and BotConfig ACTIVATE_MESSAGE_QUEUING is False but this is the main thread message.")
                    else:
                        await self.user_interaction_dispatcher.add_reaction(event=event, channel_id=event.channel_id,
                                                                            timestamp=event.timestamp,
                                                                            reaction_name=self.reaction_wait)
                    return
                else:
                    self.logger.info(
                        f"IM behavior: Message from channel {event.channel_id} discarded due to pending messages and BotConfig ACTIVATE_MESSAGE_QUEUING is False.")
            ...
            self.logger.info(
                f"IM behavior: No pending messages found. Enqueuing current message for processing in channel '{event.channel_id}', thread '{event.thread_id}'")
            # Use a constant GUID to respect the dispatcher's structure
            guid = self.STATIC_GUID  # Constant GUID defined earlier
            ...
            await self.user_interaction_dispatcher.remove_reaction(event=event, channel_id=channel_id,
                                                                   timestamp=event.timestamp,
                                                                   reaction_name=self.reaction_wait)
            await self.user_interaction_dispatcher.add_reaction(event=event, channel_id=channel_id,
                                                                timestamp=event.timestamp,
                                                                reaction_name=self.reaction_acknowledge)

            # If the event is a thread message, send a response
            if event.event_label == "thread_message":
                await self.user_interaction_dispatcher.send_message(event=event, message="",
        ...
        try:
            ...
            if event.event_label == "message":
                if self.bot_config.REQUIRE_MENTION_NEW_MESSAGE and not event.is_mention:
                    self.logger.warning(
                        "IM behavior: Message is not a mention, and the config requires direct mention for new messages. Ignoring.")
                    return
            ...
            await self.user_interaction_dispatcher.remove_reaction(event=event, channel_id=channel_id,
                                                                   timestamp=timestamp,
                                                                   reaction_name=self.reaction_acknowledge)
            await self.user_interaction_dispatcher.add_reaction(event=event, channel_id=channel_id, timestamp=timestamp,
                                                                reaction_name=self.reaction_done)
            ...
            if self.bot_config.ACTIVATE_MESSAGE_QUEUING:
                ...
                while next_message_data:
                    ...
                    if next_message_id is None and next_message_content is None:
                        ...
                        break
                    self.logger.info(
                        f"IM behavior: Found next message in the queue: {next_message_id}. Processing next message.")
                    try:
                        event_to_process = IncomingNotificationDataBase.from_json(next_message_content)
                        await self.user_interaction_dispatcher.remove_reaction(event=event_to_process, channel_id=str(
                            event_to_process.channel_id), timestamp=event_to_process.timestamp,
                                                                               reaction_name=self.reaction_wait)
                        await self.user_interaction_dispatcher.add_reaction(event=event_to_process,
                        ...
                        self.logger.error(
                            f"IM behavior: Error parsing next message: {str(e)}\n{traceback.format_exc()}")
                ...
                self.logger.info(
                    f"IM behavior: Message queuing is disabled, removing wait reaction from thread {event.thread_id}")
            ...
            self.logger.error(
                f"IM behavior: Error processing incoming notification data: {str(e)}\n{traceback.format_exc()}")
    ...
    async def begin_genai_completion(self, event: IncomingNotificationDataBase, channel_id, timestamp):
        ...
        await self.update_reaction(event=event, channel_id=channel_id, timestamp=timestamp,
                                   remove_reaction=self.reaction_writing, add_reaction=self.reaction_generating)
    ...
    async def end_genai_completion(self, event: IncomingNotificationDataBase, channel_id, timestamp):
        ...
        await self.update_reaction(event=event, channel_id=channel_id, timestamp=timestamp,
                                   remove_reaction=self.reaction_generating)
    ...
    async def begin_long_action(self, event: IncomingNotificationDataBase, channel_id, timestamp):
        ...
        await self.update_reaction(event=event, channel_id=channel_id, timestamp=timestamp,
                                   remove_reaction=self.reaction_generating, add_reaction=self.reaction_processing)
    ...
    async def end_long_action(self, event: IncomingNotificationDataBase, channel_id, timestamp):
        ...
        await self.update_reaction(event=event, channel_id=channel_id, timestamp=timestamp,
                                   remove_reaction=self.reaction_processing)
    ...
    async def mark_error(self, event: IncomingNotificationDataBase, channel_id, timestamp):
        ...
        await self.update_reaction(event=event, channel_id=channel_id, timestamp=timestamp,
                                   remove_reaction=self.reaction_generating, add_reaction=self.reaction_error)

    async def update_reaction(self, event: IncomingNotificationDataBase, channel_id, timestamp, remove_reaction,
                              add_reaction=None):
        # This method is used to update the reaction on a message.
        # It removes the specified reaction and, if provided, adds a new one.
        await self.user_interaction_dispatcher.remove_reaction(event=event, channel_id=channel_id, timestamp=timestamp,
                                                               reaction_name=remove_reaction)
        if add_reaction:
            await self.user_interaction_dispatcher.add_reaction(event=event, channel_id=channel_id, timestamp=timestamp,
                                                                reaction_name=add_reaction)
