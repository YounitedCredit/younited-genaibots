...
class ImDefaultBehaviorPlugin(UserInteractionsBehaviorBase):
    ...
    async def process_interaction(self, event_data, event_origin=None):
        try:
            ...
            thread_id = event.thread_id or event.timestamp  # Ensure unique thread_id
            ...
            self.wait_queue = self.backend_internal_queue_processing_dispatcher.wait_queue  # Use the new dispatcher
            ...
            if await self.backend_internal_queue_processing_dispatcher.has_older_messages(
                ...
                if self.global_manager.bot_config.ACTIVATE_MESSAGE_QUEUING:
                    ...
                    self.logger.info(f"IM behavior: Message from channel {channel_id} enqueued due to pending messages.")
                    return
                else:
            ...
            self.logger.info(f"IM behavior: No pending messages found. Enqueuing current message for processing in channel '{channel_id}', thread '{thread_id}'")
