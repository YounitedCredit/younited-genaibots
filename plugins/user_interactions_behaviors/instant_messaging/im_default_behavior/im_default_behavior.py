        ...
        try:
            ...
            if event.event_label == "thread_message":
                if break_keyword in event.text:
                    ...
                    await self.backend_internal_data_processing_dispatcher.write_data_content(self.backend_internal_data_processing_dispatcher.abort, abort_name, data="abort")
                    return
                    ...
                    await self.backend_internal_data_processing_dispatcher.remove_data_content(self.backend_internal_data_processing_dispatcher.abort, abort_name)
                    return
                    ...
                    # Use the InteractionQueueManager to manage queue clearing
            ...
            if await self.backend_internal_queue_processing_dispatcher.has_older_messages(
                ...
                # If message queuing is enabled, enqueue the message via the InteractionQueueManager
            ...
            # If no pending messages, enqueue the current message for processing
            method_params = {
                "channel_id": channel_id,
                "thread_id": thread_id,
                "timestamp": event.timestamp,
    ...
    async def process_incoming_notification_data(self, event: IncomingNotificationDataBase):
        try:
            ...
            if self.bot_config.ACTIVATE_MESSAGE_QUEUING:
                # Use InteractionQueueManager to dequeue the message
                await self.global_manager.interaction_queue_manager.add_to_queue(
                    "dequeue_message",
                    {
                        "channel_id": channel_id,
                ...
                while next_message_data:
                    ...
                    next_message_data = await self.global_manager.interaction_queue_manager.get_next_message(
                        data_container=self.backend_internal_queue_processing_dispatcher.messages_queue,
                        channel_id=event.channel_id,
                        thread_id=event.thread_id,
                        current_message_id=event.timestamp
