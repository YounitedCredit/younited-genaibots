...
class GenaiInteractionsImageGeneratorDispatcher(GenAIInteractionsPluginBase):
    ...
    def initialize(self, plugins: List[GenAIInteractionsPluginBase] = None):
        ...
        if self.bot_config.GENAI_IMAGE_DEFAULT_PLUGIN_NAME is not None:
            self.logger.info(
                f"Setting default Genai Image plugin to <{self.bot_config.GENAI_IMAGE_DEFAULT_PLUGIN_NAME}>")
            self.default_plugin: GenAIInteractionsPluginBase = self.get_plugin(
                self.bot_config.GENAI_IMAGE_DEFAULT_PLUGIN_NAME)
    ...
    def get_plugin(self, plugin_name=None):
        ...
        self.logger.error(
            f"GenaiInteractionsImageGeneratorDispatcher: Plugin '{plugin_name}' not found, returning default plugin")
