...
class UserInteractionsBehaviorsDispatcher(UserInteractionsBehaviorBase):
    ...
    def initialize(self, plugins: List[UserInteractionsPluginBase] = None):
        ...
        if self.bot_config.USER_INTERACTIONS_INSTANT_MESSAGING_BEHAVIOR_DEFAULT_PLUGIN_NAME is not None:
            self.logger.info(
                f"Setting default User Interactions behavior plugin to <{self.bot_config.USER_INTERACTIONS_INSTANT_MESSAGING_BEHAVIOR_DEFAULT_PLUGIN_NAME}>")
            self.default_plugin: UserInteractionsBehaviorBase = self.get_plugin(
                self.bot_config.USER_INTERACTIONS_INSTANT_MESSAGING_BEHAVIOR_DEFAULT_PLUGIN_NAME)
            self.default_plugin_name = self.default_plugin.plugin_name
        else:
            self.default_plugin = plugins[0]
            self.default_plugin_name = self.default_plugin.plugin_name
            self.logger.info(
                f"Setting User Interactions behavior default plugin to first plugin in list <{self.default_plugin_name}>")
    ...
    def get_plugin(self, plugin_name=None):
        ...
        self.logger.error(
            f"UserInteractionsInstantMessagingDispatcher: Plugin '{plugin_name}' not found, returning default plugin")
    ...
    async def begin_genai_completion(self, event: IncomingNotificationDataBase, channel_id: str, timestamp: str,
                                     plugin_name=None):
        plugin: UserInteractionsBehaviorBase = self.get_plugin(plugin_name)
        if plugin is None:
            current_method = inspect.currentframe().f_code.co_name
    ...
    async def end_genai_completion(self, event: IncomingNotificationDataBase, channel_id: str, timestamp: str,
                                   plugin_name=None):
        plugin: UserInteractionsBehaviorBase = self.get_plugin(plugin_name)
        if plugin is None:
            current_method = inspect.currentframe().f_code.co_name
    ...
    async def begin_long_action(self, event: IncomingNotificationDataBase, channel_id: str, timestamp: str,
                                plugin_name=None):
        plugin: UserInteractionsBehaviorBase = self.get_plugin(plugin_name)
        if plugin is None:
            current_method = inspect.currentframe().f_code.co_name
    ...
    async def end_long_action(self, event: IncomingNotificationDataBase, channel_id: str, timestamp: str,
                              plugin_name=None):
        plugin: UserInteractionsBehaviorBase = self.get_plugin(plugin_name)
        if plugin is None:
            current_method = inspect.currentframe().f_code.co_name
    ...
    async def begin_wait_backend(self, event: IncomingNotificationDataBase, channel_id: str, timestamp: str,
                                 plugin_name=None):
        plugin: UserInteractionsBehaviorBase = self.get_plugin(plugin_name)
        if plugin is None:
            current_method = inspect.currentframe().f_code.co_name
    ...
    async def end_wait_backend(self, event: IncomingNotificationDataBase, channel_id: str, timestamp: str,
                               plugin_name=None):
        plugin: UserInteractionsBehaviorBase = self.get_plugin(plugin_name)
        if plugin is None:
            current_method = inspect.currentframe().f_code.co_name
